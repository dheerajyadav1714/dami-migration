...
gcloud dataproc batches submit pyspark "${SCRIPT_GCS}" \
  ...
  --version="2.2" \
  --properties="spark.plugins=com.nvidia.spark.SQLPlugin,spark.rapids.sql.enabled=true,spark.task.resource.gpu.amount=1,spark.executor.resource.gpu.amount=1" \
  -- "${CSV_PATH}" "${PROJECT_ID}" "${DATASET_V3}"
